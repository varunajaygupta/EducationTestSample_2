...
public final class DateTimeUtils {
     ...
     * @return Difference in days from current time or empty string
     * @author Faisal Khan
     */
    public static String getDifferenceInDays(String givenDate) {
        ...
        try {
            ...
            double diffDays = diff / (double) (24 * 60 * 60 * 1000);
    ...
    /**
     * Returns proper date in String format - MM/DD/YYYY
     *
     * @param year
     * @param monthOfYear
     * @param dayOfMonth
    ...
    /**
     * Default "MM/dd/yyyy", if dateFormat is null
     *
     * @param date
     * @param dateFormat
     * @return Get Date from string
     */
    ...
    /**
     * Gives the system current date in MM/dd/yyyy format -- faisal
     *
    ...
    /**
     * Gives the system current time in hh:mm:ss format -- faisal
     *
     * @return system current date in string
     */
    ...
    /**
     * Gives the system current time in date object -- faisal
     *
     * @return date object
     */
    ...
    /**
     * @param time time inn string
     * @return date object
     */
    public static Date convertIntoDate(String time) {
        Calendar c = Calendar.getInstance();
        return c.getTime();
    }

    /**
     * Gives listes of dates in from today to next month last dates in the format of MM/dd/yyyy
     * like today is 08/20/2015 to it will return list of dates from 08/20/2015 to 09/30/2015 --faisal
     *
     * @param date start date
     * @return list of String
     */
    public static ArrayList<String> getDatesBetween(final String date) {

        ArrayList<String> listDates = new ArrayList<String>();
        String[] startDate = date.split("/");
        final int startDay = Integer.parseInt(startDate[1]);
        final int startMonth = Integer.parseInt(startDate[0]) - 1;
        final int startYear = Integer.parseInt(startDate[2]);

        Calendar calendar = new GregorianCalendar() {{
            set(Calendar.YEAR, startYear);
            set(Calendar.MONTH, startMonth);
            set(Calendar.DATE, startDay);
        }};

        Calendar endTime = Calendar.getInstance();
        endTime.set(startYear, startMonth, startDay, 00, 00);
        endTime.add(Calendar.MONTH, 1);
        endTime.set(Calendar.DATE, endTime.getActualMaximum(Calendar.DAY_OF_MONTH));
        SimpleDateFormat df = new SimpleDateFormat("MM/dd/yyyy");

        String[] endDate = df.format(endTime.getTime()).split("/");
        final int endDay = Integer.parseInt(endDate[1]);
        final int endMonth = Integer.parseInt(endDate[0]) - 1;
        final int endYear = Integer.parseInt(endDate[2]);

        listDates.add(date);
        while (calendar.get(Calendar.YEAR) != endYear || calendar.get(Calendar.MONTH) != endMonth || calendar.get(Calendar.DATE) != endDay) {
            ...
            listDates.add(month + "/" + day + "/" + calendar.get(Calendar.YEAR));
        }
        return listDates;
    }
}
